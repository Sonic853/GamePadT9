...
int wmain(int argc, wchar_t** argv) {
    if (argc != 2 || (std::wstring(argv[1]) != L"register" && std::wstring(argv[1]) != L"unregister")) {
        std::fwprintf(stderr, L"Usage: BridgeControl register|unregister\n"); return 2;
    }
    ...
    const char* name = action == L"register" ? "DllRegisterServer" : "DllUnregisterServer";
