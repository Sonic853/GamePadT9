...
internal sealed class Controller
{
    ...
    private long bSince;
    private bool externalCompletion, longA;
    private long aSince;
    internal void ConfigureCompletion(bool external)
    {
    ...
    public List<PadEvent> Update(Gamepad pad, long now)
    {
        ...
        if (!initialized)
        {
            ...
            aSince = now; longA = (pad.Buttons & Buttons.A) != 0;
            return events;
        }
        ...
        if (Down(Buttons.A)) { aSince = now; longA = false; }
        if (externalCompletion && (chord || (pad.Buttons & Buttons.B) != 0 || Down(Buttons.Y))) longA = true;
        ...
        if (!chord && (pad.Buttons & Buttons.B) == 0 && events.Count == 0)
        {
            ...
            else if (externalCompletion && (pad.Buttons & Buttons.A) != 0 && !longA && now - aSince >= 1000)
            { longA = true; events.Add(new(PadAction.Complete)); }
