...
设置中的摇杆用于选择九格，另一摇杆用于选择组内字母；选择左摇杆作为九格控制后，右摇杆负责字母细分。所选摇杆按下始终输入整组（左侧 L3、右侧 R3）。数字模式下不展开细分，所选摇杆按下输入 0，所选扳机输入区域数字 1–9。另一扳机不触发输入。
...
|---|---|---|
| 右摇杆 | 选择九宫格区域 | 选择 1–9 |
| 左摇杆 | 推出选择组内字母或标点，回中恢复整组 | 无输入动作 |
| RT | 输入高亮字母；回中或空白输入整组；符号格选择符号或标点 | 输入高亮数字 |
| R3（按下右摇杆） | 输入按下前的区域 | 输入 0 |
| A | 确认高亮候选 / 标点；没有待选内容时输入空格 | 没有暂存的九键待选内容时输入空格 |
| LB / RB | 上一个 / 下一个候选 | 无动作 |
